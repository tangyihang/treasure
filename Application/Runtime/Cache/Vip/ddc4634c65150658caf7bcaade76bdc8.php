...
<div class="mui-content">
    ...
    <div class="mui-row" style="margin-top:0.5rem;">
       ...
       <div class="mui-col-sm-3 mui-col-xs-3 mui-text-center mui-ellipsis" style="line-height:2rem;color:#f24646;">
             <?php echo ($v["nickname"]); ?>
       </div>
       
        <div class="mui-col-sm-2 mui-col-xs-2 mui-text-center" style="line-height:2rem;color:#f24646;">
             <?php echo ($v["sum_number_c1"]); ?>
       </div>
       
        <div class="mui-col-sm-2 mui-col-xs-2 mui-text-center" style="line-height:2rem;color:#f24646;">
             <?php echo ($v["sum_number_c2"]); ?>
       </div>
		<div class="mui-col-sm-2 mui-col-xs-2 mui-text-center" style="line-height:2rem;color:#f24646;">
             <?php echo ($v["sum_number_c3"]); ?>
       </div>
    </div>  
    <!--
       	单条记录结束
...
<div style="position:absolute;bottom:54px;z-index:9999999;background:#f24646;color:#FFF;width:100%;line-height:3rem;text-align:center;" class="jump" data-url="/vip/tui/order_detail?phone=<?php echo ($phone); ?>&time_day=<?php echo ($time_day); ?>">
	总订单记录
</div>
...
function formatData(headimgurl, nickname, sum_number_c1, sum_number_c2){
	
	var data = ' <div class="mui-row" style="margin-top:0.5rem;">'+
       '<div class="mui-col-sm-3 mui-col-xs-3 mui-text-center" style="line-height:2rem;padding-left:1rem;color:#369544;">'+
        	'<img src="'+headimgurl+'" style="width:2rem;" >'+
      '</div>'+
       '<div class="mui-col-sm-3 mui-col-xs-3 mui-text-center mui-ellipsis" style="line-height:2rem;color:#369544;">'+nickname+
       '</div>'+
        '<div class="mui-col-sm-3 mui-col-xs-3 mui-text-center" style="line-height:2rem;color:#369544;">'+sum_number_c1+
       '</div>'+
        '<div class="mui-col-sm-3 mui-col-xs-3 mui-text-center" style="line-height:2rem;color:#369544;">'+sum_number_c2+
       '</div>'+
    '</div>';
...
function pullupRefresh(){
	...
	$.post('/Vip/Tui/order_day', data, function(response){
		...
		if(response.code == 11){
			...
			{
				$('.warp').append(formatData(response.data[i]['headimgurl'], response.data[i]['nickname'],response.data[i]['sum_number_c1'], response.data[i]['sum_number_c2']));
			}
