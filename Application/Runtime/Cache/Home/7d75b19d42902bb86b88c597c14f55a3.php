...
<div class="mui-content">
   ...
   <img src="<?php echo ($output['row']['img_url']); ?>" style=" width:100%;display:block; margin:0 auto;" />
   </div>
	<!--
    	描述：轮播结束
    -->
    ...
    <div style="line-height:2.8rem;background:#FFF;">
    	<span style="background:#f24646;color:#FFF;border-radius:0.3rem;padding:0.2rem 0.5rem;margin-left:0.6rem; letter-spacing:1px; font-size:13px;">进行中</span>&nbsp;&nbsp;&nbsp;&nbsp;<span style=" letter-spacing:0.5px; font-size:13.5px; color:#333;"><?php echo ($output["row"]["name"]); ?></span>
    </div>
    <!--
    	描述：通知结束
    -->
   ...
   <!--
					 ...
					 <?php if(is_array($output["newAward"])): foreach($output["newAward"] as $key=>$v): ?><li class="mui-table-view-cell mui-media">
                                <a href="/order/orderList?phone=<?php echo ($v["phone"]); ?>">
                                    ...
                                    <div class="mui-media-body">
                                        <span style="color:#007AFF;display:inline-block;width:30%;" class='mui-ellipsis'><?php echo ($v["nickname"]); ?></span><span class="mui-pull-right" style="color:#666;"><?php echo ($v["create_time"]); ?></span>
                                        <p class='mui-ellipsis' style="line-height:1rem;font-size:12px;">刚刚获胜夺得商品<span style="color:#F00;"><?php echo ($v["goods_num"]); ?></span>单</p>
                                    </div>
                                </a>
    ...
    <div style="line-height:2rem;background:#f24646;color:#FFF;padding-left:1rem;font-size:0.7rem;" class="countDown">
    	上期幸运号段：<span style="background:#FFF;color:#FF0000;padding:0rem 0.5rem;font-weight:800;border-radius:0.2rem;"><?php echo ($output["lastNum"]); ?></span>
    	&nbsp;&nbsp;&nbsp;&nbsp;
    	...
    	<ul class="mui-table-view">
						 <?php if(is_array($output["newBuy"])): foreach($output["newBuy"] as $key=>$v): ?><li class="mui-table-view-cell mui-media">
                                <a href="/order/orderList?phone=<?php echo ($v["phone"]); ?>">
                                    ...
                                    <div class="mui-media-body">
                                        <span style="color:#007AFF;display:inline-block;width:30%;" class='mui-ellipsis'><?php echo ($v["nickname"]); ?></span><span class="mui-pull-right" style="color:#666;"><?php echo ($v["create_time"]); ?></span>
                                        <p class='mui-ellipsis' style="line-height:1rem;font-size:12px;">刚刚参与<span style="color:#F00;"><?php echo ($v["goods_num"]); ?></span>张 【<?php echo ($v["goods_name"]); ?>】</p>
                                    </div>
                                </a>
                            </li><?php endforeach; endif; ?>
...
<div style="height:auto;background:#FFF;position:fixed;bottom:54px;left:0px;width:100%;z-index:999;display:none;" id="tip">
    ...
    <div style="width:90%;margin:0.2rem 5% 1rem 5%;">
		<div style="width:100%;margin-top:0.5rem;margin-bottom:0.5rem;">
			<button type="button" class="mui-btn mui-btn-warning sub"  data-pay=3 style="width:100%; background: #f24646; border: 1px #f24646 solid;" id="jfzf">积分支付（积分余额:<?php echo ($output["member"]["points"]); ?>）</button>
		</div>
...
<div style="position:fixed;bottom:54px;left:0px;color:#FFF;text-align:center;line-height:2.5rem;width:100%;z-index:10;background:#FFF;padding-top:0.5rem;border-top:1px solid #E4E3E6;">
		<div style="width:50%;float:left;">
    		<button type="button" class="mui-btn mui-btn-primary" data-loading-text="提交中" style="width:80%; background: #2f99e3; border: 1px #2f99e3 solid;" id="small">买小 ( <?php echo ($output["rowCata"]["small"]); ?> )</button>
        </div>
        <div style="width:50%;float:left;">
    		<button type="button" class="mui-btn mui-btn-danger" data-loading-text="提交中" style="width:80%; background: #f24646; border: 1px #f24646 solid;" id="big">买大 ( <?php echo ($output["rowCata"]["big"]); ?> )</button>
        </div>	
</div>
...
function redi(){
	 url = '/order/orderList?phone=<?php echo ($output["phone"]); ?>';
	 window.location.href=url;
}
</script>
<script>
var mask 		= mui.createMask();//callback为用户点击蒙版时自动执行的回调；
var goods_id 	= <?php echo ($output["row"]["id"]); ?>;
var price   	= <?php echo ($output["rowCata"]["price"]); ?>;

var cata_id		= <?php echo ($output["rowCata"]["id"]); ?>;
var snatch_type = '';
