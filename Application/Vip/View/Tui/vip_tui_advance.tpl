...
<div class="mui-content">
        ...
        <div class="mui-col-sm-12 mui-col-xs-12 mui-text-center" style="height:12rem;background:#f24646;background-size:100% auto;">
            <div class="mui-row" style="margin-top:0.5rem;">
                <div class="mui-col-sm-12 mui-col-xs-12" style="text-align:center;">
                    ...
                    <p style="color:#FFF;font-size:1rem;position:relative;">
                        <span class="sp" style="position:absolute;left:0.5rem;color:#fff;"><< 问题受理</span>
                        <empty name="output['sumDay'][0]['sum_price']">
                            0
                        </empty>{{$output['sumDay'][0]['sum_price']}}
                        <span style="position:absolute;right:0.5rem;"><a style="color:#FFF;" href="/vip/tui/user?phone={{$phone}}">用户信息 >></a></span>
                    </p>
                </div>
    ...
    <div id="pullrefresh" class="mui-content mui-scroll-wrapper" style="margin-top:13rem;touch-action: none;">
        <div class="mui-scroll">
            <div class="warp">
                <foreach name="output.resultList" item="v">
                    <!--
                    单条记录开始-->
                    <div class="mui-row jump" style="background:#FFF;margin-top:0.5rem;"
                         data-url="/vip/tui/order_day?phone={{$phone}}&time_day={{$v['time_day']}}">
    ...
    $.post('/Vip/Tui/tui_advance', data, function (response) {
      ...
      if (response.code == 11) {
        ...
        for (var i = 0; i < response.data.length; i++) {
          var sum = parseFloat(response.data[i]['sum_number_c1']) + parseFloat(response.data[i]['sum_number_c2']) + parseFloat(response.data[i]['sum_number_c3']);
